document.addEventListener('DOMContentLoaded', function() {
  ...
  checkOutBtn.addEventListener('click', async function() {
    try {
      const response = await fetch('/api/employee/check-out', {
        method: 'POST',
        headers
      });
  ...
  // Fetch notifications - Modified to check if element exists
  async function fetchNotifications() {
    try {
      // Check if notifications list element exists before proceeding
      const notificationsListElement = document.getElementById('notifications-list');
      if (!notificationsListElement) {
        console.log('Notifications list element not found - skipping notifications fetch');
  ...
  // Update notifications UI - Modified to check if element exists
  function updateNotificationsUI(notifications) {
    // Check if notifications list element exists before proceeding
    const notificationsList = document.getElementById('notifications-list');
    if (!notificationsList) {
      console.log('Notifications list element not found - skipping UI update');
      return; // Exit the function if the element doesn't exist
    }
  ...
  async function initPage() {
    ...
    // Only fetch notifications if the element exists
